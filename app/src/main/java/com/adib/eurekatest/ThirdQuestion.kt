...
class ThirdQuestion(val item: Question): Fragment() {
    ...
    private fun showResult(answer: String) {
        if (answer == item.correctAnswer) com.checkThree(answer)
        else com.checkThree(answer)
    }
}